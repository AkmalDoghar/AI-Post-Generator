...
Make each draft visibly native to ${platformSpec.name}, not a lightly edited LinkedIn post. Change the opening rhythm, paragraph density, CTA style, and use 1-3 tasteful platform-appropriate emojis. Avoid repeating the same emoji across all three drafts.
...
`;
      ...
      if (parsed && Array.isArray(parsed.drafts) && parsed.drafts.length > 0) {
        const enrichedDrafts = parsed.drafts.map((d) => {
          const finalPostText = ensureSourceLinks(d.postText, summary);
          const quality = evaluatePostQuality(finalPostText, summary, storyAnalysis);
          return {
            ...d,
    ...
    const finalPostText = ensureSourceLinks(postText, summary);
    const quality = evaluatePostQuality(finalPostText, summary, storyAnalysis);
    const draftItem = {
      id: "draft-a",
 ...
 */
function buildFallbackVariations({ summary, storyAnalysis, angle, platform, tone = "professional", length = "medium" }) {
  ...
  const platformStyle = {
    linkedin: { lead: "This week I pushed", emoji: ["🚀", "🧠", "🔗"], cta: "Explore the work:" },
    twitter: { lead: "Shipping log:", emoji: ["⚡", "🧵", "👇"], cta: "Open-source trail:" },
    instagram: { lead: "Build diary:", emoji: ["🎨", "✨", "📸"], cta: "See what I built:" },
    facebook: { lead: "A quick progress update:", emoji: ["👋", "🛠️", "💬"], cta: "Take a look:" },
  }[platform] || { lead: "This week I pushed", emoji: ["🚀", "🧠", "🔗"], cta: "Explore the work:" };

  const toneStyle = {
    professional: "A focused shipping update:",
    casual: "A small update from the build:",
    technical: "A technical note from this week's work:",
    storytelling: "The story behind this week's work:",
    personal: "A builder's note from this week:",
  }[tone] || "A focused shipping update:";

  const lengthDetail = {
    short: "Keeping this one focused: useful progress without unnecessary complexity.",
    medium: "The goal was useful progress without unnecessary complexity, while keeping the next change easier to make.",
    long: "The goal was useful progress without unnecessary complexity. That meant looking beyond individual commits and making the surrounding product experience, structure, and next steps easier to understand.",
  }[length] || "The goal was useful progress without unnecessary complexity.";

  // Variation A — Story-Driven
  const textA = `${platformStyle.emoji[0]} ${toneStyle}

${platformStyle.lead} ${repoNames} a little further.
...
Still building. Still learning. ${platformStyle.emoji[1]}

${repoLinks ? `\n${platformStyle.cta}\n${repoLinks}\n` : ""}
#BuildInPublic #WebDevelopment #${languages.replace(/[^a-zA-Z]/g, "")} #DeveloperLife`;

  // Variation B — Technical Deep-Dive
  const textB = `${platformStyle.emoji[1]} ${toneStyle}

One of the main focuses while working on ${repoNames} this week was improving code structure and maintainability.
...
Clean code > fast code. ${platformStyle.emoji[2]}

${repoLinks ? `\nGitHub source:\n${repoLinks}\n` : ""}
#SoftwareEngineering #CodeQuality #WebDev #TypeScript`;

  // Variation C — Build in Public
  const textC = `${platformStyle.emoji[2]} ${toneStyle}

Building in public recap · Week of steady shipping
...
What are you shipping this week? Let me know below! ${platformStyle.emoji[0]}
...
function ensureSourceLinks(postText, summary) {
  if (!postText || !summary) return postText;

  const links = [
    ...(summary.reposTouched || []).slice(0, 3).map((repo) => `https://github.com/${repo}`),
    ...(summary.commits || []).map((commit) => commit.url).filter(Boolean).slice(0, 2),
  ].filter((link, index, allLinks) => link && allLinks.indexOf(link) === index);

  const missingLinks = links.filter((link) => !postText.includes(link));
  if (!missingLinks.length) return postText;
