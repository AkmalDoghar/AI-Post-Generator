...
export async function fetchActivitySummary(username, days = 7) {
  ...
  for (const event of recent) {
    ...
    if (event.type === "PushEvent") {
      for (const c of event.payload?.commits || []) {
        commits.push({ repo: event.repo.name, message: c.message, url: `https://github.com/${event.repo.name}/commit/${c.sha}` });
      }
    }
  ...
  // GitHub can delay PushEvents for newly created repositories, so use the
  // public commits endpoint as a fallback when the event feed has no commits.
  if (!commits.length) {
    let reposRes = await fetch(`${GITHUB_API}/users/${username}/repos?sort=pushed&per_page=20&type=owner`, { headers: authHeaders() });
    if (reposRes.status === 401 && process.env.GITHUB_TOKEN) {
      reposRes = await fetch(`${GITHUB_API}/users/${username}/repos?sort=pushed&per_page=20&type=owner`, { headers: publicHeaders() });
    }
    if (reposRes.ok) {
      const repos = await reposRes.json();
      const commitResults = await Promise.all(repos.slice(0, 10).map(async (repo) => {
        try {
          let response = await fetch(`${GITHUB_API}/repos/${repo.full_name}/commits?author=${encodeURIComponent(username)}&since=${since.toISOString()}&per_page=25`, { headers: authHeaders() });
          if (response.status === 401 && process.env.GITHUB_TOKEN) {
            response = await fetch(`${GITHUB_API}/repos/${repo.full_name}/commits?author=${encodeURIComponent(username)}&since=${since.toISOString()}&per_page=25`, { headers: publicHeaders() });
          }
      ...
      for (let index = 0; index < commitResults.length; index += 1) {
        for (const commit of commitResults[index]) {
          commits.push({ repo: repos[index]?.full_name || "repository", message: commit.commit?.message || "", url: commit.html_url || "" });
        }
      }
    }
  }
